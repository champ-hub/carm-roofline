...
insert_save_active_mem_elem_count(void *drcontext, instrlist_t *ilist, instr_t *where, opnd_t ref, reg_id_t base,
                 reg_id_t scratch, opnd_t hey)
{
	opnd_size_t elem_sz = opnd_get_vector_element_size(hey);
	scratch = reg_resize_to_opsz(scratch, OPSZ_8);
		...
		bool for_trace, bool translating,void *user_data)
{
	...
	if (countopcodes) {
		...
		if (instr_fetch != NULL) {
			...
			if (!(is_sve_instr(instr_fetch) && is_predicated_instr(instr_fetch)) ) // Need to check SVE mem instruction on instr_fetch and not instr_operands as it is NULL for instr_operands ( on v.10.93.19887)
				return DR_EMIT_DEFAULT;

			if (instr_reads_memory(instr_fetch) || instr_writes_memory(instr_fetch)) {
				/* Insert code to add an entry for each memory reference opnd. */
				for (i = 0; i < instr_num_srcs(instr_fetch); i++) {
					const opnd_t src = instr_get_src(instr_fetch, i);
					if (opnd_is_memory_reference(src)) {
						instrument_sve_predicated_mem(drcontext, bb, instr,instr_fetch,instr_get_dst(instr_fetch, 0));
					}
				}
				for (i = 0; i < instr_num_dsts(instr_fetch); i++) {
					const opnd_t dst = instr_get_dst(instr_fetch, i);
					if (opnd_is_memory_reference(dst) ) {
						instrument_sve_predicated_mem(drcontext, bb, instr,instr_fetch, instr_get_src(instr_fetch, 0));
					}
				}
			}
