...
typedef struct _sve_predicated_instr_t {
    ushort type; /* equal to 0 if floating-point instruction, 1 if memory instruction , 2 if integer instruction */
...
count_sve_predicated_instructions(void *drcontext)
{
    ...
    for (sve_predicated_instr = (sve_predicated_instr_t *)data->buf_base; sve_predicated_instr < buf_ptr; sve_predicated_instr++) {
        if (sve_predicated_instr->type == REF_TYPE_FPINS) { 	// Is a floating point instruction
            data->counters->fp_ops += sve_predicated_instr->elem_size * sve_predicated_instr->active_elem_count;
        }
        if (sve_predicated_instr->type == REF_TYPE_MEMINS) { 				// Is a mem instruction 
            data->counters->bytes += sve_predicated_instr->elem_size * sve_predicated_instr->active_elem_count;
        }
        if (sve_predicated_instr->type == REF_TYPE_INTINS) { 	// Is a int ops instruction
...
is_flop (instr_t *instr)
{
    ...
    return (     opc == OP_fabd 
            ...
            || (opc >= OP_fdivr && opc <= OP_fnmls)
            || (opc >= OP_fsubr && opc <= OP_fmsb)
            || (opc == OP_fnmsb)
...
opnd_t get_sve_vector_operand(instr_t *instr)
{
    reg_id_t reg;
    int i;
    for (i = 0; i < instr_num_srcs(instr); i++) {
...
int n_flops(instr_t *instr)
{
    ...
    int value;
    value = is_fma(instr);
    if (is_neon_instr(instr)) {
        value *= number_of_elements_in_neon_vector(instr); 
    }
    return value;    
}

/* Return number of INTOP done by an integer instruction (SVE predicated instructions not supported) */
int n_intops(instr_t *instr)
{
    void *drcontext = dr_get_current_drcontext();
    int value;
    value = is_int_fma(instr);
...
insert_save_active_elem_count(void *drcontext, instrlist_t *ilist, instr_t *where, opnd_t ref, reg_id_t base,
        reg_id_t scratch)
{
    opnd_size_t elem_sz = opnd_get_vector_element_size(get_sve_vector_operand(where));

    scratch = reg_resize_to_opsz(scratch, OPSZ_8);

    instr_t *instr = INSTR_CREATE_cntp_sve_pred(drcontext, opnd_create_reg(scratch),opnd_create_reg_element_vector(opnd_get_reg(ref),elem_sz),opnd_create_reg_element_vector(opnd_get_reg(ref),elem_sz));
    MINSERT(ilist, where,
            instr);
    insert_load_buf_ptr(drcontext, ilist, where, base);
    MINSERT(ilist, where,
    ...
    static void
insert_save_active_mem_elem_count(void *drcontext, instrlist_t *ilist, instr_t *where, opnd_t ref, reg_id_t base,
        reg_id_t scratch, opnd_t vec_opnd)
{
    opnd_size_t elem_sz = opnd_get_vector_element_size(vec_opnd);
    scratch = reg_resize_to_opsz(scratch, OPSZ_8);
...
instrument_sve_predicated_fpins(void *drcontext, instrlist_t *ilist, instr_t *where,instr_t *instr, ushort flop_per_elem)
{
    ...
    insert_save_type(drcontext, ilist, where, reg_ptr, reg_tmp, REF_TYPE_FPINS);
...
instrument_sve_predicated_intins(void *drcontext, instrlist_t *ilist, instr_t *where,instr_t *instr, ushort flop_per_elem)
{
    ...
    insert_save_active_elem_count(drcontext, ilist, where, get_predicate_operand(instr), reg_ptr, reg_tmp);
    insert_save_type(drcontext, ilist, where, reg_ptr, reg_tmp, REF_TYPE_INTINS);
    insert_save_elem_size(drcontext, ilist, where, reg_ptr, reg_tmp, flop_per_elem);
    insert_update_buf_ptr(drcontext, ilist, where, reg_ptr, sizeof(sve_predicated_instr_t));
...
instrument_sve_predicated_mem(void *drcontext, instrlist_t *ilist, instr_t *where,instr_t *instr, opnd_t ref)
{
    ...
    insert_save_active_mem_elem_count(drcontext, ilist, where, get_predicate_operand(instr), reg_ptr, reg_tmp, ref);
    insert_save_elem_size(drcontext, ilist, where, reg_ptr, reg_tmp, opnd_size_in_bytes(opnd_get_vector_element_size(ref)));
    insert_save_type(drcontext, ilist, where, reg_ptr, reg_tmp, REF_TYPE_MEMINS);
    insert_update_buf_ptr(drcontext, ilist, where, reg_ptr, sizeof(sve_predicated_instr_t));
    /* Restore scratch registers */
    if (drreg_unreserve_register(drcontext, ilist, where, reg_ptr) != DRREG_SUCCESS ||
            drreg_unreserve_register(drcontext, ilist, where, reg_tmp) != DRREG_SUCCESS)
...

{
    ...
    for (cursor = instrlist_first(bb); cursor != NULL;
        ...
        if (countopcodes) {
            ...
            if (is_intop(cursor)) {
                ...
                counter->int_ops += n_intops(cursor);
                continue;
            }
            if (is_flop(cursor)) {
                counter->fp_ins++;
    ...
    if (counter->fp_ops != 0 || counter->bytes != 0 || counter->int_ins != 0)
        ...
        bool for_trace, bool translating,void *user_data)
{
    ...
    if (countopcodes) {
        ...
        if (instr_fetch != NULL) {
            ...
            if (!(is_sve_instr(instr_fetch) && is_predicated_instr(instr_fetch)) ) // Need to check SVE mem instruction on instr_fetch and not instr_operands as it is NULL for instr_operands ( on DynamoRIO v.10.93.19887)
                return DR_EMIT_DEFAULT;

            if (instr_reads_memory(instr_fetch) || instr_writes_memory(instr_fetch)) {

                instrument_sve_predicated_mem(drcontext, bb, instr,instr_fetch,get_sve_vector_operand(instr_fetch));
            }
                ...
                if (is_intop(instr_operands)) 
                    instrument_sve_predicated_intins(drcontext, bb, instr,instr_operands, is_int_fma(instr_operands));

                if (is_flop(instr_operands)) 
                    instrument_sve_predicated_fpins(drcontext, bb, instr,instr_operands, is_fma(instr_operands));
            }
...
dr_client_main(client_id_t id, int argc, const char *argv[])
{
    ...
    sprintf(filename,"carm_dbi_output_%d.txt",dr_get_process_id());
