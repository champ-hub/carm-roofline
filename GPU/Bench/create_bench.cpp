...
void create_benchmark_flops(int device, string target, string operation, string precision, int threads_per_block, int num_blocks, int iterations) {
	...
	if (target == "cuda") {
		...
		while(getline(input, text)) {
			...
			if (text == "//DEFINE KERNEL PARAMETERS") {
				...
				string aux;
				if (precision == "sp") aux = "float";
				else if (precision == "dp") aux = "double";
				else if (precision == "int") aux = "int";
				else if (precision == "hp") aux = "__half";
				else if (precision == "bf16") exit(5);
		...
		// automate make of test, not run, run on top level
	} else {
		// Tensor
	}
}