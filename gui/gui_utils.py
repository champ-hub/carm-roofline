...
def read_application_csv_file(file_path):
    if not os.path.exists(file_path):
        print("Application file does not exist:", file_path)  # noqa: T201
        return False
    ...
    try:
        with open(file_path, newline="") as csvfile:
            ...
            if header is None:
                print("File is empty:", file_path)  # noqa: T201
                return False
        ...
        print("Failed to read the file:", file_path, "Error:", e)  # noqa: T201
        return False
...
def calculate_roofline(values, min_ai):
    ...
    dots = {}
    peak_flops = values[5] if values[5] > 0 else values[4]
...
def plot_roofline(values, dots, name_suffix, isa, line_legend, line_size, line_legend_detailed):
    ...
    if values[4] > 0:
        ...
        linedash = "solid" if values[5] == 0 else "dashdot"
        trace_inst = go.Scatter(
            ...
            hovertemplate="<b>%{text}</b><br>(%{x}, %{y})<br><extra></extra>",
            line={"color": color_inst, "dash": linedash, "width": line_size},
            name=legend_text,
            showlegend=line_legend,
        )
    ...
    if values[5] > 0:
        ...
        trace_inst = go.Scatter(
            ...
            hovertemplate="<b>%{text}</b><br>(%{x}, %{y})<br><extra></extra>",
            line={"color": color_inst, "dash": "solid", "width": line_size},
            name=legend_text,
            showlegend=line_legend,
        )
        traces.append(trace_inst)
...
def draw_annotation(
    ...
    if cache_level in cache_levels and cache_level in lines and lines[cache_level]["ridge"][0] > 0:
    ...
    if cache_level in cache_levels and values[cache_levels.index(cache_level)] > 0 and cache_level in lines:
        aidots[0] = 0.00390625
